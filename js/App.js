...
const driverColors=driversDirectory.reduce((acc,driver)=>{// Use driver.name as primary key since that's what route.driver stores
const name=driver.name||driver.firstName||'';acc[name]={bg:driver.bgColor||'#f5f5f5',border:driver.borderColor||'#9e9e9e',header:driver.borderColor||'#757575'};// Also add firstName as key for backwards compatibility
if(driver.firstName&&driver.firstName!==name){acc[driver.firstName]=acc[name];}return acc;},{'':{bg:'#f5f5f5',border:'#9e9e9e',header:'#757575'}});const handleSetUserName=name=>{setUserName(name);localStorage.setItem('userName',name);setShowNameModal(false);};const today=new Date().toLocaleDateString('en-US',{weekday:'long',year:'numeric',month:'short',day:'numeric'});const calculateTotals=useCallback(()=>{let totalPallets=0;let truckSpots=0;const byType={};// Initialize counts for all pallet types
palletTypes.forEach(pType=>{byType[pType.abbrev]=0;});routes.forEach(route=>{route.stores.forEach(store=>{const types=store.palletTypes&&store.palletTypes.length>0?store.palletTypes:Array(12).fill(palletTypes[0]?.abbrev||'FZ');const links=store.palletLinks||[];(store.pallets||[]).forEach((p,idx)=>{// Count both numeric values AND "?" pending pallets
if(p&&typeof p==='number'&&p>0||p==='?'){totalPallets++;if(!links[idx])truckSpots++;const storedType=types[idx]||palletTypes[0]?.abbrev||'FZ';// Handle both old format (F, D, S) and new format (FZ, DR, SP)
const normalizedType=storedType==='F'?'FZ':storedType==='D'?'DR':storedType==='S'?'SP':storedType;byType[normalizedType]=(byType[normalizedType]||0)+1;}});});});return{routes:routes.length,totalPallets,truckSpots,byType};},[routes,palletTypes]);const getDriverStats=()=>{const stats={};routes.forEach(route=>{if(route.driver&&route.driver!==''){if(!stats[route.driver]){stats[route.driver]=0;}stats[route.driver]++;}});return stats;};// Load routes for invoice date range from database
const loadInvoiceRoutes=useCallback(async(startDate,endDate)=>{await loadInvoiceRoutesForRange({supabase,startDate,endDate,setInvoiceRoutes,setInvoiceRoutesLoading});},[]);useEffect(()=>{if(invoiceDateRange.start&&invoiceDateRange.end){loadInvoiceRoutes(invoiceDateRange.start,invoiceDateRange.end);}},[invoiceDateRange.start,invoiceDateRange.end,loadInvoiceRoutes]);const calculateInvoiceData=useCallback((startDate,endDate)=>calculateInvoiceDataFromRoutes({startDate,endDate,invoiceRoutes,storesDirectory,invoiceRateOverrides}),[invoiceRoutes,storesDirectory,invoiceRateOverrides]);const calculateRouteTotal=(stores,palletCount=8,palletTypesConfig=[])=>{const defaultType=palletTypesConfig[0]?.abbrev||'FZ';return stores.reduce((acc,store)=>{const types=store.palletTypes&&store.palletTypes.length>0?store.palletTypes:Array(palletCount).fill(defaultType);const links=store.palletLinks||[];const pallets=store.pallets||[];// Count pallets by type dynamically (include "?" pending pallets)
const byType={...acc.byType};palletTypesConfig.forEach(pType=>{const count=pallets.reduce((c,val,idx)=>{const storedType=types[idx]||defaultType;// Handle both old format (F, D, S) and new format (FZ, DR, SP)
const normalizedType=storedType==='F'?'FZ':storedType==='D'?'DR':storedType==='S'?'SP':storedType;// Count both numeric values AND "?" pending pallets
const hasPallet=val&&typeof val==='number'&&val>0||val==='?';return c+(hasPallet&&normalizedType===pType.abbrev?1:0);},0);byType[pType.abbrev]=(byType[pType.abbrev]||0)+count;});// Calculate total pallets (count all pallets with values, including "?")
let tot=0;let linkedCount=0;for(let i=0;i<pallets.length;i++){if(pallets[i]&&typeof pallets[i]==='number'&&pallets[i]>0||pallets[i]==='?'){tot++;if(links[i])linkedCount++;}}return{tc:acc.tc+(store.tc||0),byType,tot:acc.tot+tot,// Actual pallet count
truckSpots:(acc.truckSpots||0)+(tot-linkedCount),// Truck spots (for Print Manifest)
fresh:acc.fresh+(store.fresh||0)};},{tc:0,byType:{},tot:0,truckSpots:0,fresh:0});};// ─────────────────────────────────────────────────────────────────────────
// ╔╗
...
// ╚
// ─────────────────────────────────────────────────────────────────────────
const addStoreToDirectory=()=>{hasPendingChanges.current=true;lastInteractionTime.current=Date.now();const newId=Math.max(...storesDirectory.map(s=>s.id),0)+1;setStoresDirectory(prev=>[...prev,{id:newId,code:'',name:'New Store',street:'',city:'',state:'NY',zip:'',hours:'',maxTrailer:"53'",manager:'',phone:'',email:''}]);};const updateStoreDirectory=(storeId,field,value)=>{// Mark pending changes to prevent real-time sync from overwriting
hasPendingChanges.current=true;lastInteractionTime.current=Date.now();console.log('updateStoreDirectory called - hasPendingChanges set to true');setStoresDirectory(prev=>prev.map(s=>{if(s.id!==storeId)return s;// Ensure code, zip, and phone are always strings
const safeValue=['code','zip','phone'].includes(field)?String(value):value;return{...s,[field]:safeValue};}));};const removeStoreFromDirectory=storeId=>{const store=storesDirectory.find(s=>s.id===storeId);const storeName=store?`${String(store.code||'')} - ${store.name||''}`:'Unknown Store';setDeleteConfirmModal({type:'Store',id:storeId,name:storeName,onConfirm:()=>{hasPendingChanges.current=true;lastInteractionTime.current=Date.now();setStoresDirectory(prev=>prev.filter(s=>s.id!==storeId));// Log activity
logActivity('delete','store_directory',String(storeId),storeName,null,storeName,null,null);}});};// Import stores from Excel/CSV file
const importStoresFromFile=event=>{const file=event.target.files[0];if(!file)return;const reader=new FileReader();// Get existing store codes for duplicate check
const existingCodes=new Set(storesDirectory.map(s=>String(s.code).trim().toLowerCase()));if(file.name.endsWith('.csv')){// Handle CSV
reader.onload=e=>{try{const text=e.target.result;const rows=parseCsvText(text);if(rows.length===0){alert('No stores found in file. Please check the format.');return;}const headers=rows[0].map(h=>h.trim().toLowerCase().replace(/['"]/g,''));const newStores=[];const skippedCodes=[];const maxId=Math.max(...storesDirectory.map(s=>s.id),0);let addedCount=0;for(let i=1;i<rows.length;i++){const values=rows[i].map(v=>v.trim().replace(/['"]/g,''));if(values.length<2)continue;const storeCode=values[headers.indexOf('store code')]||values[headers.indexOf('code')]||values[0]||'';// Check for duplicate code
if(existingCodes.has(String(storeCode).trim().toLowerCase())){skippedCodes.push(storeCode);continue;}addedCount++;const store={id:maxId+addedCount,code:String(storeCode),name:values[headers.indexOf('store name')]||values[headers.indexOf('name')]||values[1]||'',street:values[headers.indexOf('street')]||values[headers.indexOf('address')]||values[2]||'',city:values[headers.indexOf('city')]||values[3]||'',state:values[headers.indexOf('state')]||values[4]||'NY',zip:String(values[headers.indexOf('zip')]||values[headers.indexOf('zipcode')]||values[5]||''),hours:values[headers.indexOf('hours')]||values[6]||'',maxTrailer:values[headers.indexOf('max trailer')]||values[headers.indexOf('maxtrailer')]||values[7]||"53'",manager:values[headers.indexOf('manager')]||values[8]||'',phone:String(values[headers.indexOf('phone')]||values[9]||''),email:values[headers.indexOf('email')]||values[10]||''};newStores.push(store);}if(newStores.length>0){setStoresDirectory(prev=>[...prev,...newStores]);}// Show result message
let message='';if(newStores.length>0){message+=`✅ Imported ${newStores.length} new stores.\n`;}if(skippedCodes.length>0){message+=`⚠ Skipped ${skippedCodes.length} duplicate codes: ${skippedCodes.slice(0,10).join(', ')}${skippedCodes.length>10?'...':''}`;}if(newStores.length===0&&skippedCodes.length===0){message='No stores found in file. Please check the format.';}alert(message);}catch(err){console.error('Import error:',err);alert('Failed to import file. Please check the format.');}};reader.readAsText(file);}else{// Handle Excel (.xlsx, .xls)
reader.onload=async e=>{try{// Dynamically load SheetJS library
if(!window.XLSX){const script=document.createElement('script');script.src='https://unpkg.com/xlsx@0.18.5/dist/xlsx.full.min.js';document.head.appendChild(script);await new Promise(resolve=>script.onload=resolve);}const data=new Uint8Array(e.target.result);const workbook=window.XLSX.read(data,{type:'array'});const sheetName=workbook.SheetNames[0];const worksheet=workbook.Sheets[sheetName];const jsonData=window.XLSX.utils.sheet_to_json(worksheet);const maxId=Math.max(...storesDirectory.map(s=>s.id),0);const newStores=[];const skippedCodes=[];let addedCount=0;jsonData.forEach(row=>{const storeCode=row['Store Code']||row['STORE CODE']||row['Code']||row['code']||'';// Check for duplicate code
if(existingCodes.has(String(storeCode).trim().toLowerCase())){skippedCodes.push(storeCode);return;}addedCount++;newStores.push({id:maxId+addedCount,code:String(storeCode),name:row['Store Name']||row['STORE NAME']||row['Name']||row['name']||'',street:row['Street']||row['STREET']||row['Address']||row['address']||'',city:row['City']||row['CITY']||row['city']||'',state:row['State']||row['STATE']||row['state']||'NY',zip:String(row['Zip']||row['ZIP']||row['ZipCode']||row['zip']||''),hours:row['Hours']||row['HOURS']||row['hours']||'',maxTrailer:row['Max Trailer']||row['MAX TRAILER']||row['MaxTrailer']||row['maxTrailer']||"53'",manager:row['Manager']||row['MANAGER']||row['manager']||'',phone:String(row['Phone']||row['PHONE']||row['phone']||''),email:row['Email']||row['EMAIL']||row['email']||''});});if(newStores.length>0){setStoresDirectory(prev=>[...prev,...newStores]);}// Show result message
let message='';if(newStores.length>0){message+=`✅ Imported ${newStores.length} new stores.\n`;}if(skippedCodes.length>0){message+=`⚠ Skipped ${skippedCodes.length} duplicate codes: ${skippedCodes.slice(0,10).join(', ')}${skippedCodes.length>10?'...':''}`;}if(newStores.length===0&&skippedCodes.length===0){message='No stores found in file. Please check the format.';}alert(message);}catch(err){console.error('Import error:',err);alert('Failed to import Excel file. Please check the format.');}};reader.readAsArrayBuffer(file);}event.target.value='';// Reset input
};// ─────────────────────────────────────────────────────────────────────────
// ╔╗
...
// ╚
// ─────────────────────────────────────────────────────────────────────────
const addDriverToDirectory=()=>{hasPendingChanges.current=true;lastInteractionTime.current=Date.now();const newId=Math.max(...driversDirectory.map(d=>d.id),0)+1;setDriversDirectory(prev=>[...prev,{id:newId,name:'',firstName:'',lastName:'',dob:'',phone:'',email:'',license:'CDL-A',dlNumber:'',dlState:'',licenseExp:'',mcExp:'',truck:'',status:'Active',hireDate:''}]);};const updateDriverDirectory=(driverId,field,value)=>{// Mark pending changes to prevent real-time sync from overwriting
hasPendingChanges.current=true;lastInteractionTime.current=Date.now();setDriversDirectory(prev=>prev.map(driver=>{if(driver.id!==driverId)return driver;const updated={...driver,[field]:value};// Auto-update name when firstName or lastName changes
if(field==='firstName'||field==='lastName'){updated.name=`${field==='firstName'?value:driver.firstName} ${field==='lastName'?value:driver.lastName}`.trim()||updated.firstName||updated.lastName;}return updated;}));};const removeDriverFromDirectory=driverId=>{const driver=driversDirectory.find(d=>d.id===driverId);setDeleteConfirmModal({type:'Driver',id:driverId,name:driver?`${driver.firstName} ${driver.lastName}`.trim()||driver.name:'Unknown Driver',onConfirm:()=>{hasPendingChanges.current=true;lastInteractionTime.current=Date.now();setDriversDirectory(prev=>prev.filter(d=>d.id!==driverId));}});};// Get drivers with expiring documents
const getExpiringDriverDocuments=(daysThreshold=30)=>{const today=new Date();const threshold=new Date(today.getTime()+daysThreshold*24*60*60*1000);const expiring=[];driversDirectory.forEach(driver=>{if(driver.status!=='Active')return;// Check License Expiration
if(driver.licenseExp){const licenseDate=new Date(driver.licenseExp);if(licenseDate<=threshold){const daysLeft=Math.ceil((licenseDate-today)/(1000*60*60*24));expiring.push({driver,type:'License',expDate:driver.licenseExp,daysLeft,expired:daysLeft<0});}}// Check MC Expiration
if(driver.mcExp){const mcDate=new Date(driver.mcExp);if(mcDate<=threshold){const daysLeft=Math.ceil((mcDate-today)/(1000*60*60*24));expiring.push({driver,type:'Medical Card',expDate:driver.mcExp,daysLeft,expired:daysLeft<0});}}});// Sort by days left (most urgent first)
return expiring.sort((a,b)=>a.daysLeft-b.daysLeft);};// Send expiration alert email
const sendExpirationAlertEmail=async(alerts,recipients)=>{if(!recipients||recipients.length===0){setInfoModal({type:'warning',title:'No Recipients Selected',message:'Please select at least one user to receive the alert email.'});return;}try{// Get current session for secure Edge Function call
const{data:{session}}=await supabase.auth.getSession();if(!session){setInfoModal({type:'error',title:'Authentication Required',message:'You must be logged in to send alerts. Please refresh and log in again.'});return;}const response=await fetch(SUPABASE_URL+'/functions/v1/send-expiration-alerts',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+session.access_token},body:JSON.stringify({alerts,recipients:recipients.map(r=>({email:r.email,name:r.name})),org_name:'Newmans Refrigerated Service'})});// Check if function exists
if(response.status===404){throw new Error('Edge Function not deployed. Deploy using: supabase functions deploy send-expiration-alerts');}const result=await response.json();if(!response.ok||result.error){throw new Error(result.error||result.message||`HTTP ${response.status}`);}// Close the expiration alerts modal first
setExpirationAlertsModal(false);setSelectedAlertRecipients([]);// Then show success modal
setInfoModal({type:'success',title:'Alerts Sent!',message:`Expiration alert emails have been sent successfully.`,details:[{label:'Alerts Sent',value:alerts.length},{label:'Recipients',value:recipients.map(r=>r.name||r.email).join(', ')}]});}catch(err){console.error('Failed to send alerts:',err);const errorMsg=err.message||'Unknown error';// Check for common error types
if(errorMsg.includes('Failed to fetch')||errorMsg.includes('NetworkError')){setInfoModal({type:'warning',title:'Edge Function Not Found',message:'The send-expiration-alerts Edge Function is not deployed.',note:'Deploy the function using: supabase functions deploy send-expiration-alerts'});}else if(errorMsg.includes('not configured')||errorMsg.includes('RESEND_API_KEY')){setInfoModal({type:'warning',title:'Email Not Configured',message:'The Resend API key is not set in Supabase secrets.',note:'Run: supabase secrets set RESEND_API_KEY=re_xxxxx'});}else{setInfoModal({type:'warning',title:'Email Failed',message:errorMsg,note:'Check the browser console for more details.'});}}};// Import drivers from Excel/CSV file
const importDriversFromFile=event=>{const file=event.target.files[0];if(!file)return;const reader=new FileReader();if(file.name.endsWith('.csv')){reader.onload=e=>{try{const text=e.target.result;const rows=parseCsvText(text);if(rows.length===0){alert('No drivers found in file. Please check the format.');return;}const headers=rows[0].map(h=>h.trim().toLowerCase().replace(/['"]/g,''));const newDrivers=[];const maxId=Math.max(...driversDirectory.map(d=>d.id),0);for(let i=1;i<rows.length;i++){const values=rows[i].map(v=>v.trim().replace(/['"]/g,''));if(values.length<2)continue;const firstName=values[headers.indexOf('first name')]||values[headers.indexOf('firstname')]||values[0]||'';const lastName=values[headers.indexOf('last name')]||values[headers.indexOf('lastname')]||values[1]||'';const driver={id:maxId+i,name:`${firstName} ${lastName}`.trim(),firstName,lastName,dob:values[headers.indexOf('dob')]||values[headers.indexOf('date of birth')]||'',phone:values[headers.indexOf('phone')]||values[2]||'',email:values[headers.indexOf('email')]||values[3]||'',license:values[headers.indexOf('license')]||values[headers.indexOf('license type')]||values[4]||'CDL-A',dlNumber:values[headers.indexOf('dl #')]||values[headers.indexOf('dl number')]||'',dlState:values[headers.indexOf('state')]||values[headers.indexOf('dl state')]||'',licenseExp:values[headers.indexOf('license exp')]||values[headers.indexOf('licenseexp')]||values[5]||'',mcExp:values[headers.indexOf('mc exp')]||values[headers.indexOf('mc expiration')]||'',truck:values[headers.indexOf('truck')]||values[6]||'',status:values[headers.indexOf('status')]||values[7]||'Active',hireDate:values[headers.indexOf('hire date')]||values[headers.indexOf('hiredate')]||values[8]||''};newDrivers.push(driver);}if(newDrivers.length>0){setDriversDirectory(prev=>[...prev,...newDrivers]);alert(`Successfully imported ${newDrivers.length} drivers!`);}else{alert('No drivers found in file. Please check the format.');}}catch(err){console.error('Import error:',err);alert('Failed to import file. Please check the format.');}};reader.readAsText(file);}else{reader.onload=async e=>{try{if(!window.XLSX){const script=document.createElement('script');script.src='https://unpkg.com/xlsx@0.18.5/dist/xlsx.full.min.js';document.head.appendChild(script);await new Promise(resolve=>script.onload=resolve);}const data=new Uint8Array(e.target.result);const workbook=window.XLSX.read(data,{type:'array'});const sheetName=workbook.SheetNames[0];const worksheet=workbook.Sheets[sheetName];const jsonData=window.XLSX.utils.sheet_to_json(worksheet);const maxId=Math.max(...driversDirectory.map(d=>d.id),0);const newDrivers=jsonData.map((row,idx)=>{const firstName=row['First Name']||row['FIRST NAME']||row['FirstName']||row['firstName']||'';const lastName=row['Last Name']||row['LAST NAME']||row['LastName']||row['lastName']||'';return{id:maxId+idx+1,name:row['Name']||row['NAME']||`${firstName} ${lastName}`.trim(),firstName,lastName,dob:row['DOB']||row['Dob']||row['Date of Birth']||row['DATE OF BIRTH']||'',phone:row['Phone']||row['PHONE']||row['phone']||'',email:row['Email']||row['EMAIL']||row['email']||'',license:row['License']||row['LICENSE']||row['License Type']||'CDL-A',dlNumber:row['DL #']||row['DL Number']||row['DL']||row['Driver License']||'',dlState:row['State']||row['STATE']||row['DL State']||'',licenseExp:row['License Exp']||row['LICENSE EXP']||row['LicenseExp']||'',mcExp:row['MC Exp']||row['MC EXP']||row['MC Expiration']||'',truck:row['Truck']||row['TRUCK']||row['truck']||'',status:row['Status']||row['STATUS']||row['status']||'Active',hireDate:row['Hire Date']||row['HIRE DATE']||row['HireDate']||''};});if(newDrivers.length>0){setDriversDirectory(prev=>[...prev,...newDrivers]);alert(`Successfully imported ${newDrivers.length} drivers!`);}else{alert('No drivers found in file. Please check the format.');}}catch(err){console.error('Import error:',err);alert('Failed to import Excel file. Please check the format.');}};reader.readAsArrayBuffer(file);}event.target.value='';};// ─────────────────────────────────────────────────────────────────────────
// ╔╗
// ║  SECTION 10: EQUIPMENT OPERATIONS                                           ║
// ║  Trucks, Trailers, Tractors - add, update, remove                           ║
// ╚
// ─────────────────────────────────────────────────────────────────────────
const addTruckToDirectory=()=>{hasPendingChanges.current=true;lastInteractionTime.current=Date.now();const newId=Math.max(...trucksDirectory.map(t=>t.id),0)+1;setTrucksDirectory(prev=>[...prev,{id:newId,number:'',zone:'Zone A',vin:'',make:'Freightliner',model:'',year:'',plate:'',plateExp:'',maxTrailer:"53'",maxPallets:22,status:'Active',mileage:0,lastService:'',nextService:'',assignedDriver:'',notes:''}]);};const updateTruckDirectory=(truckId,field,value)=>{// Mark pending changes to prevent real-time sync from overwriting
hasPendingChanges.current=true;lastInteractionTime.current=Date.now();setTrucksDirectory(prev=>prev.map(truck=>{if(truck.id!==truckId)return truck;return{...truck,[field]:value};}));};const removeTruckFromDirectory=truckId=>{const truck=trucksDirectory.find(t=>t.id===truckId);setDeleteConfirmModal({type:'Truck',id:truckId,name:truck?`#${truck.number} - ${truck.make} ${truck.model}`:'Unknown Truck',onConfirm:()=>{hasPendingChanges.current=true;lastInteractionTime.current=Date.now();setTrucksDirectory(prev=>prev.filter(t=>t.id!==truckId));}});};// Trailer Directory functions
const addTrailerToDirectory=()=>{hasPendingChanges.current=true;lastInteractionTime.current=Date.now();const newId=Math.max(...trailersDirectory.map(t=>t.id),0)+1;setTrailersDirectory(prev=>[...prev,{id:newId,number:'',type:'Refrigerated',size:"53'",vin:'',make:'Utility',model:'',year:'',plate:'',plateExp:'',maxPallets:22,status:'Active',notes:''}]);};const updateTrailerDirectory=(trailerId,field,value)=>{// Mark pending changes to prevent real-time sync from overwriting
hasPendingChanges.current=true;lastInteractionTime.current=Date.now();setTrailersDirectory(prev=>prev.map(trailer=>{if(trailer.id!==trailerId)return trailer;return{...trailer,[field]:value};}));};const removeTrailerFromDirectory=trailerId=>{const trailer=trailersDirectory.find(t=>t.id===trailerId);setDeleteConfirmModal({type:'Trailer',id:trailerId,name:trailer?`#${trailer.number} - ${trailer.size} ${trailer.type}`:'Unknown Trailer',onConfirm:()=>{hasPendingChanges.current=true;lastInteractionTime.current=Date.now();setTrailersDirectory(prev=>prev.filter(t=>t.id!==trailerId));}});};// Tractor Directory functions
const addTractorToDirectory=()=>{hasPendingChanges.current=true;lastInteractionTime.current=Date.now();const newId=Math.max(...tractorsDirectory.map(t=>t.id),0)+1;setTractorsDirectory(prev=>[...prev,{id:newId,number:'',vin:'',make:'Freightliner',model:'',year:'',plate:'',plateExp:'',mileage:0,status:'Active',assignedTrailer:'',notes:''}]);};const updateTractorDirectory=(tractorId,field,value)=>{// Mark pending changes to prevent real-time sync from overwriting
hasPendingChanges.current=true;lastInteractionTime.current=Date.now();setTractorsDirectory(prev=>prev.map(tractor=>{if(tractor.id!==tractorId)return tractor;return{...tractor,[field]:value};}));};const removeTractorFromDirectory=tractorId=>{const tractor=tractorsDirectory.find(t=>t.id===tractorId);setDeleteConfirmModal({type:'Tractor',id:tractorId,name:tractor?`#${tractor.number} - ${tractor.make} ${tractor.model}`:'Unknown Tractor',onConfirm:()=>{hasPendingChanges.current=true;lastInteractionTime.current=Date.now();setTractorsDirectory(prev=>prev.filter(t=>t.id!==tractorId));}});};// ─────────────────────────────────────────────────────────────────────────
// ╔╗
...
const importData=event=>{const file=event.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=e=>{try{const data=JSON.parse(e.target.result);if(data.routesByDate){setRoutesByDate(data.routesByDate);}else if(data.routes){// Handle old format
const today=new Date().toLocaleDateString('en-CA');setRoutesByDate({[today]:data.routes});}if(data.storesDirectory)setStoresDirectory(data.storesDirectory);if(data.driversDirectory)setDriversDirectory(data.driversDirectory);if(data.trucksDirectory)setTrucksDirectory(data.trucksDirectory);if(data.trailersDirectory)setTrailersDirectory(data.trailersDirectory);if(data.tractorsDirectory)setTractorsDirectory(data.tractorsDirectory);if(data.palletTypes)setPalletTypes(data.palletTypes);alert('Data imported successfully!');}catch(err){alert('Failed to import data. Please check the file format.');}};reader.readAsText(file);event.target.value='';};const totals=calculateTotals();const driverStats=getDriverStats();const getSyncStatusDisplay=()=>{// Check actual online status first
if(!navigator.onLine){return{text:'⚠ OFFLINE',color:'#f44336'};}if(needsReload){return{text:'Reloading...',color:'#ff9800'};}switch(syncStatus){case'connecting':return{text:'Connecting...',color:'#ff9800'};case'syncing':return{text:'Saving...',color:'#2196f3'};case'synced':return{text:'Synced',color:'#4caf50'};case'error':return{text:'Connection Error',color:'#f44336'};default:return{text:'Unknown',color:'#999'};}};const statusDisplay=getSyncStatusDisplay();// ─────────────────────────────────────────────────────────────────────────
// ╔╗
