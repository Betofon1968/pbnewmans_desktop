export default function OverviewTab(props){
  const {
    ...
    is,
    isAdmin,
    isCardRouteConfirmed,
    isFirst,
    isLabelRouteConfirmed,
    isLast,
    isLoadOrderConfirmed,
    isRouteConfirmedManifest,
    isRouteSummaryConfirmed,
    item,
    justify,
    label,
    last,
  ...
  return React.createElement(React.Fragment,null,
    /*#__PURE__*/React.createElement("div",{style:{position:'sticky',top:0,zIndex:100,display:'flex',gap:'4px',padding:'4px 16px',height:'26px',boxSizing:'content-box',background:'white',borderBottom:'1px solid #e0e0e0',alignItems:'center',fontSize:'10px',justifyContent:'space-between'}},/*#__PURE__*/React.createElement("div",{style:{display:'flex',alignItems:'center',gap:'8px',minWidth:0,flexWrap:'nowrap',overflowX:'auto',paddingBottom:'1px'}},/*#__PURE__*/React.createElement("span",{style:{fontWeight:600,color:'#1a7f4b',fontSize:'10px',whiteSpace:'nowrap'}},totals.routes," Routes"),/*#__PURE__*/React.createElement("span",{style:{color:'#ddd'}},"\u2022"),/*#__PURE__*/React.createElement("span",{style:{fontWeight:600,color:'#1a7f4b',fontSize:'10px',whiteSpace:'nowrap'}},totals.truckSpots,"/",totals.totalPallets," PLT"),/*#__PURE__*/React.createElement("span",{style:{color:'#ddd'}},"\u2022"),palletTypes.filter(pt=>(totals.byType?.[pt.abbrev]||0)>0).map(pt=>/*#__PURE__*/React.createElement("span",{key:'type-'+pt.abbrev,style:{fontWeight:700,color:pt.color||'#666',fontSize:'10px',whiteSpace:'nowrap'}},pt.abbrev,":",totals.byType?.[pt.abbrev]||0)),Object.entries(driverStats||{}).sort((a,b)=>a[0].localeCompare(b[0])).map(([driver,count])=>{const driverInfo=driversDirectory.find(d=>d.name===driver||`${d.firstName||''} ${d.lastName||''}`.trim()===driver)||{};const borderColor=driverInfo.borderColor||'#999';const bgColor=driverInfo.bgColor||'white';return/*#__PURE__*/React.createElement("span",{key:'driver-'+driver,style:{padding:'1px 10px',border:`2px solid ${borderColor}`,borderRadius:'8px',fontWeight:600,color:borderColor,background:bgColor,fontSize:'10px',lineHeight:'16px',whiteSpace:'nowrap'}},driver,": ",count);})),/*#__PURE__*/React.createElement("div",{style:{display:'flex',gap:'4px',alignItems:'center'}},/*#__PURE__*/React.createElement("select",{id:"printOrientation",defaultValue:localStorage.getItem('printOrientation')||'portrait',onChange:e=>localStorage.setItem('printOrientation',e.target.value),style:{padding:'2px 4px',borderRadius:'3px',border:'1px solid #ddd',fontSize:'10px',cursor:'pointer'}},/*#__PURE__*/React.createElement("option",{value:"portrait"},"Portrait"),/*#__PURE__*/React.createElement("option",{value:"landscape"},"Landscape")),/*#__PURE__*/React.createElement("label",{style:{display:'flex',alignItems:'center',gap:'2px',fontSize:'10px',cursor:'pointer'},title:"Fit to page"},/*#__PURE__*/React.createElement("input",{type:"checkbox",id:"fitToPage",defaultChecked:localStorage.getItem('printFitToPage')==='true',onChange:e=>localStorage.setItem('printFitToPage',e.target.checked),style:{cursor:'pointer',width:'11px',height:'11px'}}),"Fit"),/*#__PURE__*/React.createElement("label",{style:{display:'flex',alignItems:'center',gap:'2px',fontSize:'10px',cursor:'pointer',background:'#fff3e0',padding:'1px 4px',borderRadius:'3px',border:'1px solid #ff9800'},title:"Group by route"},/*#__PURE__*/React.createElement("input",{type:"checkbox",id:"groupByRouteNum",defaultChecked:localStorage.getItem('printGroupByRoute')==='true',onChange:e=>localStorage.setItem('printGroupByRoute',e.target.checked),style:{cursor:'pointer',width:'11px',height:'11px'}}),"Grp"),/*#__PURE__*/React.createElement("label",{style:{display:'flex',alignItems:'center',gap:'2px',fontSize:'10px',cursor:'pointer',background:'#e3f2fd',padding:'1px 4px',borderRadius:'3px',border:'1px solid #2196f3'},title:"Hide empty columns"},/*#__PURE__*/React.createElement("input",{type:"checkbox",id:"hideEmptyCols",defaultChecked:localStorage.getItem('printHideEmpty')==='true',onChange:e=>localStorage.setItem('printHideEmpty',e.target.checked),style:{cursor:'pointer',width:'11px',height:'11px'}}),"Hide∅"),/*#__PURE__*/React.createElement("button",{onClick:()=>{// Get options
...
let maxPalletsByType={};palletTypes.forEach(pt=>{maxPalletsByType[pt.abbrev]=0;});routes.forEach(route=>{route.stores.forEach(store=>{const types=store.palletTypes||[];const countByType={};palletTypes.forEach(pt=>{countByType[pt.abbrev]=0;});(store.pallets||[]).forEach((p,idx)=>{// Include both numeric values AND "?" pending pallets
if(p!==null&&p!==undefined&&p!==''){const pType=types[idx]||palletTypes[0]?.abbrev||'FZ';const normalized=pType==='F'?'FZ':pType==='D'?'DR':pType==='S'?'SP':pType;countByType[normalized]=(countByType[normalized]||0)+1;}});palletTypes.forEach(pt=>{if(countByType[pt.abbrev]>maxPalletsByType[pt.abbrev]){maxPalletsByType[pt.abbrev]=countByType[pt.abbrev];}});});});// Filter to only pallet types that have at least one pallet
...
const routeTotalByType={};palletTypes.forEach(pt=>{routeTotalByType[pt.abbrev]=0;});route.stores.forEach(store=>{const types=store.palletTypes||[];const links=store.palletLinks||[];(store.pallets||[]).forEach((p,idx)=>{// Include both numeric values AND "?" pending pallets
if(p!==null&&p!==undefined&&p!==''){routeTotalPallets++;if(links[idx]===true)routeLinkedCount++;// Only add numeric values to cases total
...
(store.pallets||[]).forEach((p,idx)=>{// Include both numeric values AND "?" pending pallets
if(p!==null&&p!==undefined&&p!==''){const pType=types[idx]||palletTypes[0]?.abbrev||'FZ';const normalized=pType==='F'?'FZ':pType==='D'?'DR':pType==='S'?'SP':pType;const isLinked=links[idx]===true;// This pallet is linked to previous
