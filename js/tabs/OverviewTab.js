import {guardPastDatePrintWithToday} from '../modules/utils/pastDatePrintGuard.js';
...
function OverviewTab(props){
  ...
  const normalizeDriverName = React.useCallback((name) => (
    String(name || '').replace(/\s+/g, ' ').trim()
  ), []);
  const scrollToDriverRoute = React.useCallback((driverName) => {
    const normalizedDriverName = normalizeDriverName(driverName);
    const targetRoute = routes.find((route) => (
      normalizeDriverName(route.driver) === normalizedDriverName
    ));
    if (!targetRoute) return;

    if (!expandedRoutes.includes(targetRoute.id)) {
      toggleRouteExpand(targetRoute.id);
    }

    window.requestAnimationFrame(() => {
      const routeElement = document.getElementById(`route-${targetRoute.id}`);
      if (!routeElement) return;

      routeElement.scrollIntoView({ behavior: 'smooth', block: 'start' });
      routeElement.animate?.(
        [
          { boxShadow: '0 0 0 0 rgba(26,127,75,0)' },
          { boxShadow: '0 0 0 6px rgba(26,127,75,0.28)' },
          { boxShadow: '0 0 0 0 rgba(26,127,75,0)' },
        ],
        { duration: 950, easing: 'ease-out' }
      );
    });
  }, [expandedRoutes, normalizeDriverName, routes, toggleRouteExpand]);
  return React.createElement(React.Fragment,null,
    /*#__PURE__*/React.createElement("div",{style:{position:'sticky',top:0,zIndex:100,display:'flex',gap:'4px',padding:'4px 16px',height:'26px',boxSizing:'content-box',background:'white',borderBottom:'1px solid #e0e0e0',alignItems:'center',fontSize:'10px',justifyContent:'space-between'}},/*#__PURE__*/React.createElement("div",{style:{display:'flex',alignItems:'center',gap:'8px',minWidth:0,flexWrap:'nowrap',overflowX:'auto',paddingBottom:'1px'}},/*#__PURE__*/React.createElement("span",{style:{fontWeight:600,color:'#1a7f4b',fontSize:'10px',whiteSpace:'nowrap'}},totals.routes," Routes"),/*#__PURE__*/React.createElement("span",{style:{color:'#ddd'}},"\u2022"),/*#__PURE__*/React.createElement("span",{style:{fontWeight:600,color:'#1a7f4b',fontSize:'10px',whiteSpace:'nowrap'}},totals.truckSpots,"/",totals.totalPallets," PLT"),/*#__PURE__*/React.createElement("span",{style:{color:'#ddd'}},"\u2022"),palletTypes.filter(pt=>(totals.byType?.[pt.abbrev]||0)>0).map(pt=>/*#__PURE__*/React.createElement("span",{key:'type-'+pt.abbrev,style:{fontWeight:700,color:pt.color||'#666',fontSize:'10px',whiteSpace:'nowrap'}},pt.abbrev,":",totals.byType?.[pt.abbrev]||0)),Object.entries(driverStats||{}).sort((a,b)=>a[0].localeCompare(b[0])).map(([driver,count])=>{const driverInfo=driversDirectory.find(d=>d.name===driver||`${d.firstName||''} ${d.lastName||''}`.trim()===driver)||{};const borderColor=driverInfo.borderColor||'#999';const bgColor=driverInfo.bgColor||'white';return/*#__PURE__*/React.createElement("span",{key:'driver-'+driver,onClick:()=>scrollToDriverRoute(driver),onKeyDown:e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();scrollToDriverRoute(driver);}},role:"button",tabIndex:0,title:`Go to ${driver}'s route`,style:{padding:'1px 10px',border:`2px solid ${borderColor}`,borderRadius:'8px',fontWeight:600,color:borderColor,background:bgColor,fontSize:'10px',lineHeight:'16px',whiteSpace:'nowrap',cursor:'pointer',userSelect:'none'}},driver,": ",count);})),/*#__PURE__*/React.createElement("div",{style:{display:'flex',gap:'4px',alignItems:'center'}},/*#__PURE__*/React.createElement("select",{id:"printOrientation",defaultValue:safeStorageGet('printOrientation','portrait'),onChange:e=>safeStorageSet('printOrientation',e.target.value),style:{padding:'2px 4px',borderRadius:'3px',border:'1px solid #ddd',fontSize:'10px',cursor:'pointer'}},/*#__PURE__*/React.createElement("option",{value:"portrait"},"Portrait"),/*#__PURE__*/React.createElement("option",{value:"landscape"},"Landscape")),/*#__PURE__*/React.createElement("label",{style:{display:'flex',alignItems:'center',gap:'2px',fontSize:'10px',cursor:'pointer'},title:"Fit to page"},/*#__PURE__*/React.createElement("input",{type:"checkbox",id:"fitToPage",defaultChecked:safeStorageGet('printFitToPage','false')==='true',onChange:e=>safeStorageSet('printFitToPage',e.target.checked),style:{cursor:'pointer',width:'11px',height:'11px'}}),"Fit"),/*#__PURE__*/React.createElement("label",{style:{display:'flex',alignItems:'center',gap:'2px',fontSize:'10px',cursor:'pointer',background:'#fff3e0',padding:'1px 4px',borderRadius:'3px',border:'1px solid #ff9800'},title:"Group by route"},/*#__PURE__*/React.createElement("input",{type:"checkbox",id:"groupByRouteNum",defaultChecked:safeStorageGet('printGroupByRoute','false')==='true',onChange:e=>safeStorageSet('printGroupByRoute',e.target.checked),style:{cursor:'pointer',width:'11px',height:'11px'}}),"Grp"),/*#__PURE__*/React.createElement("label",{style:{display:'flex',alignItems:'center',gap:'2px',fontSize:'10px',cursor:'pointer',background:'#e3f2fd',padding:'1px 4px',borderRadius:'3px',border:'1px solid #2196f3'},title:"Hide empty columns"},/*#__PURE__*/React.createElement("input",{type:"checkbox",id:"hideEmptyCols",defaultChecked:safeStorageGet('printHideEmpty','false')==='true',onChange:e=>safeStorageSet('printHideEmpty',e.target.checked),style:{cursor:'pointer',width:'11px',height:'11px'}}),"Hide∅"),/*#__PURE__*/React.createElement("button",{onClick:async()=>{const ok=await guardPastDatePrintWithToday({kind:'Manifest',selectedDate,todayStr:getTodayInTimezone(),setPastDatePrintModal});if(!ok)return;// Get options
